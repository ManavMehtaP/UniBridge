...
export function hodScope(req: Request, _res: Response, next: NextFunction) {
  if (!req.user?.isHod) {
  ...
  const activeSemesterId =
    ...
    store.semesters.find(
      (item) => item.universityId === req.user?.universityId && item.isActive,
    )?.id;

  const scopes = store.hodBatchScopes.filter(
    (scope) => scope.facultyId === req.user?.id && (!activeSemesterId || scope.semesterId === activeSemesterId),
  );
