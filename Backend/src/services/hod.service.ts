...
export class HodService {
  getMyScope(userId: string, universityId: string, hodBatchIds: string[], hodSemesterIds: string[]) {
    const hod = this.findHod(userId, universityId);
    const activeSemester =
      store.semesters.find((item) => hodSemesterIds.includes(item.id) && item.isActive) ??
    ...
    const batches = hodBatchIds
      .map((batchId) => {
        ...
        const studentCount = store.studentEnrollments.filter(
          (item) => item.batchId === batch.id && item.semesterId === activeSemester.id && item.isCurrent,
        ).length;
    ...
    const totalFaculty = new Set(
      store.facultyBatchAssignments
        .filter(
          (item) => hodBatchIds.includes(item.batchId) && hodSemesterIds.includes(item.semesterId),
        )
  ...
  getDashboardSummary(hodBatchIds: string[], hodSemesterIds: string[]) {
    const scopedEnrollments = store.studentEnrollments.filter(
      (item) => hodBatchIds.includes(item.batchId) && item.isCurrent,
    );
    ...
    const avgAttendance =
      totalStudents === 0
        ...
        : Number(
            (
              scopedEnrollments.reduce((sum, item) => sum + item.attendancePct, 0) / totalStudents
            ).toFixed(1),
          );
    ...
    const resultsUploadedPct =
      totalStudents === 0
        ? 0
        : Math.round(
            (scopedEnrollments.filter((item) => item.avgMarksPct > 0).length / totalStudents) * 100,
          );
  ...
  getStudents(params: StudentListParams) {
    ...
    const currentEnrollments = store.studentEnrollments.filter(
      (item) =>
        item.isCurrent &&
    ...
    const enrollmentByStudentId = new Map<string, StudentEnrollment>();
  ...
  private findHod(userId: string, universityId: string): Faculty {
    const hod = store.faculties.find(
      (item) => item.id === userId && item.universityId === universityId && item.isHod,
    );
