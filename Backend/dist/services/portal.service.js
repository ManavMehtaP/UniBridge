...
function getActiveSemester(universityId) {
...
function getSemester(semesterId, universityId) {
...
function statusFromAttendanceAndMarks(attendancePct, avgMarksPct, isActive = true) {
    if (!isActive) {
        return "INACTIVE";
    }
...
function currentEnrollmentForStudent(studentId, semesterId) {
    return store.studentEnrollments.find((item) => item.studentId === studentId && item.isCurrent && (!semesterId || item.semesterId === semesterId));
}
...
function studentByEnrollmentNo(enrollmentNo) {
...
function facultyByEmployeeId(employeeId) {
...
function facultyById(facultyId) {
...
function ensureBatchInScope(batchId, scope) {
...
function scopedCurrentEnrollments(scope, semesterId) {
    return store.studentEnrollments.filter((item) => item.isCurrent &&
        scope.hodBatchIds.includes(item.batchId) &&
        (!semesterId || item.semesterId === semesterId));
}
function allAttendanceForEnrollment(enrollmentId, semesterId) {
    return store.attendanceRecords.filter((item) => item.enrollmentId === enrollmentId && (!semesterId || item.semesterId === semesterId));
}
...
function getMentorAssignment(studentId, semesterId) {
    return store.mentorAssignments.find((item) => item.studentId === studentId && item.semesterId === semesterId);
}
function getScopedFaculty(scope) {
    const facultyIds = new Set(store.facultyBatchAssignments
...
export const portalService = {
    login(email, password, role) {
        if (role === "STUDENT") {
            const student = store.students.find((item) => (item.email === email || item.enrollmentNo === email) && item.password === password);
            if (!student) {
        ...
        const resolvedRole = role === "SUPER_ADMIN" ? "SUPER_ADMIN" : faculty.isHod ? "HOD" : "FACULTY";
    ...
    register(body, universityId) {
        ...
        if (body.role === "STUDENT") {
            if (!body.enrollmentNo || !body.branch || !body.admissionYear) {
                throw new ApiError(400, "VALIDATION_ERROR", "Student registration requires enrollmentNo, branch, and admissionYear.");
            }
            if (store.students.some((item) => item.email === body.email || item.enrollmentNo === body.enrollmentNo)) {
        ...
        if (!body.employeeId || !body.department) {
            throw new ApiError(400, "VALIDATION_ERROR", "Faculty/HOD registration requires employeeId and department.");
        }
        if (store.faculties.some((item) => item.email === body.email || item.employeeId === body.employeeId)) {
    ...
    myScope(scope) {
        ...
        const batches = scope.hodBatchIds.map((batchId) => {
            ...
            const studentCount = scopedCurrentEnrollments(scope, activeSemester.id).filter((item) => item.batchId === batchId).length;
            return {
                id: batch.id,
                code: batch.code,
    ...
    dashboardAttendanceTrend(_scope, months = 6) {
    ...
    dashboardResultsOverview(scope, semesterId) {
    ...
    dashboardActivityFeed(scope, page = 1, limit = 10) {
    ...
    listStudents(scope, query) {
        const page = Number(query.page ?? 1);
        const limit = Number(query.limit ?? 20);
        const semester = getSemester(query.semesterId, scope.universityId);
    ...
    getStudentHistory(scope, enrollmentNo) {
        const student = studentByEnrollmentNo(enrollmentNo);
    ...
    updateStudent(scope, enrollmentNo, body) {
    ...
    updateStudentStatus(enrollmentNo, isActive) {
        const student = studentByEnrollmentNo(enrollmentNo);
    ...
    listFaculty(scope, query) {
        const page = Number(query.page ?? 1);
        const limit = Number(query.limit ?? 10);
        const rows = getScopedFaculty(scope)
            .map((item) => {
            const subjectCount = new Set(store.facultyBatchAssignments.filter((entry) => entry.facultyId === item.id && entry.subjectId).map((entry) => entry.subjectId)).size;
    ...
    updateMentorCode(employeeId, mentorCode) {
        const faculty = facultyByEmployeeId(employeeId);
        if (faculty.isHod) {
    ...
    updateFacultyStatus(employeeId, isActive) {
        const faculty = facultyByEmployeeId(employeeId);
    ...
    createFacultyAssignment(body) {
        if (store.facultyBatchAssignments.some((item) => item.facultyId === body.facultyId &&
    ...
    resultsUploadContext(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
        const year = getAcademicYear(semester.academicYearId);
        return {
    ...
    resultsStudents(scope, semesterId, batchId, subjectId) {
        ensureBatchInScope(batchId, scope);
    ...
    resultsManual(body) {
        ...
        body.results.forEach((item) => {
            const existing = store.results.find((entry) => entry.enrollmentId === item.enrollmentId && entry.phaseId === body.phaseId && entry.subjectId === body.subjectId);
            if (existing) {
    ...
    resultsPreview(phaseId, subjectId, batchId) {
        const rows = store.results.filter((item) => item.phaseId === phaseId && item.subjectId === subjectId && item.batchId === batchId);
        return {
    ...
    resultsPublish(phaseId, subjectId, batchId) {
        const rows = store.results.filter((item) => item.phaseId === phaseId && item.subjectId === subjectId && item.batchId === batchId);
        if (rows.length === 0) {
    ...
    resultsUploadHistory(scope, page = 1, limit = 10) {
    ...
    resultsPhaseStatus(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
        ...
        return {
            phases: store.phases
                .filter((item) => item.semesterId === semester.id)
                .map((phase) => {
                const subjectsUploaded = new Set(latestResults(scope, undefined, phase.id).map((item) => item.subjectId)).size;
                return {
                    phase: phase.label,
                    subjectsTotal,
                    subjectsUploaded,
    ...
    updateResult(resultId, marksObtained, grade) {
    ...
    attendanceSummary(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
        const records = store.attendanceRecords.filter((item) => item.semesterId === semester.id && scope.hodBatchIds.includes(item.batchId));
    ...
    attendanceHeatmap(scope, batchId, semesterId) {
        ensureBatchInScope(batchId, scope);
    ...
    attendanceTable(scope, batchId, semesterId, search, page = 1, limit = 20) {
        ensureBatchInScope(batchId, scope);
        const rows = store.studentEnrollments
            ...
            const perSubjectEntries = store.subjects
                ...
                .map((subject) => {
                const record = store.attendanceRecords.find((item) => item.enrollmentId === enrollment.id && item.subjectId === subject.id);
                return [subject.code, record ? Math.round(percentage(record)) : 0];
            });
    ...
    attendanceBySubject(scope, batchId, semesterId) {
        ensureBatchInScope(batchId, scope);
        return {
    ...
    facultyAttendance(body, facultyId) {
        const assignment = store.facultyBatchAssignments.find((item) => item.facultyId === facultyId && item.batchId === body.batchId && item.subjectId === body.subjectId);
        if (!assignment) {
            throw new ApiError(403, "NOT_ASSIGNED_TO_BATCH", "Faculty is not assigned to this batch.");
        }
        body.attendance.forEach((item) => {
            const record = store.attendanceRecords.find((entry) => entry.enrollmentId === item.enrollmentId && entry.subjectId === body.subjectId);
            if (record?.isLocked) {
    ...
    lockAttendance(subjectId, batchId, semesterId) {
        const rows = store.attendanceRecords.filter((item) => item.subjectId === subjectId && item.batchId === batchId && item.semesterId === semesterId);
        rows.forEach((item) => {
            item.isLocked = true;
        });
        return { lockedCount: rows.length, isLocked: true };
    },
    unlockAttendance(enrollmentId, subjectId) {
    ...
    lockAllAttendance(batchId, semesterId) {
    ...
    attendanceExport(scope, batchId, semesterId) {
    ...
    studentAttendance(userId, semesterId) {
        const student = store.students.find((item) => item.id === userId);
        if (!student) {
            throw new ApiError(404, "NOT_FOUND", "Student not found.");
        }
    ...
    listSubjects(scope, semesterId, search, type) {
        const semester = getSemester(semesterId, scope.universityId);
    ...
    copySubjects(fromSemesterId, toSemesterId) {
    ...
    mentorshipSummary(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
    ...
    mentorshipMentors(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
        const assignments = store.mentorAssignments.filter((item) => item.semesterId === semester.id);
    ...
    mentorshipAssignments(scope, query) {
        const semester = getSemester(query.semesterId, scope.universityId);
    ...
    mentorshipUnassigned(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
        const assignedStudentIds = new Set(store.mentorAssignments.filter((item) => item.semesterId === semester.id).map((item) => item.studentId));
    ...
    assignMentor(studentEnrollmentNo, facultyId, semesterId) {
    ...
    reassignMentor(studentEnrollmentNo, newFacultyId, semesterId) {
        const student = studentByEnrollmentNo(studentEnrollmentNo);
    ...
    autoAssignMentors(scope, semesterId) {
    ...
    analyticsAttendanceTrend(scope, months = 6) {
    ...
    analyticsAttendanceBySubject(scope, semesterId, batchId) {
        const semester = getSemester(semesterId, scope.universityId);
        const filtered = this.attendanceBySubject(scope, batchId ?? scope.hodBatchIds[0], semester.id);
        return filtered;
    },
    analyticsAttendanceDistribution(scope, batchId) {
        const enrollments = scopedCurrentEnrollments(scope).filter((item) => !batchId || item.batchId === batchId);
    ...
    analyticsMarksBySubject(scope, phaseId, batchId) {
        return {
            subjects: store.subjects
    ...
    analyticsGradeDistribution(scope, phaseId, batchId) {
    ...
    analyticsLeaderboard(scope, phaseId, batchId, limit = 10) {
        const rows = scopedCurrentEnrollments(scope)
            .filter((item) => !batchId || item.batchId === batchId)
            .map((item) => {
            const student = store.students.find((entry) => entry.id === item.studentId);
    ...
    analyticsPerformanceRadar(scope, phaseId) {
    ...
    notifyAtRiskMentor(enrollmentNo) {
        const student = studentByEnrollmentNo(enrollmentNo);
    ...
    promotionPreview(fromAcademicYearId, toAcademicYearId) {
    ...
    savePromotionMapping(fromAcademicYearId, toAcademicYearId, mappings) {
        const draftId = `draft_${Date.now()}`;
        const draft = { id: draftId, fromAcademicYearId, toAcademicYearId, mappings };
        store.promotionDrafts.push(draft);
        return { savedCount: mappings.length, draftId };
    },
    suggestRollNumbers(draftId) {
    ...
    promotionPreviewSummary(draftId) {
        const draft = store.promotionDrafts.find((item) => item.id === draftId);
        if (!draft) {
            throw new ApiError(404, "NOT_FOUND", "Draft not found.");
        }
        const byBatch = Object.entries(draft.mappings.reduce((acc, mapping) => {
    ...
    executePromotion(draftId, mappings) {
        const draft = store.promotionDrafts.find((item) => item.id === draftId);
        if (!draft) {
            throw new ApiError(404, "NOT_FOUND", "Draft not found.");
        }
    ...
    phaseTimeline(scope, semesterId) {
        const semester = getSemester(semesterId, scope.universityId);
    ...
    updateSettingsProfile(userId, body) {
        const faculty = facultyById(userId);
        faculty.name = body.name ?? faculty.name;
        faculty.email = body.email ?? faculty.email;
        faculty.phone = body.phone ?? faculty.phone;
    ...
    universitySettings(universityId) {
    ...
    addUniversityBranch(universityId, code, name) {
        const university = getUniversity(universityId);
    ...
    updateAttendanceRules(body) {
        store.attendanceRules = body;
        return store.attendanceRules;
    },
    resetMentorAssignments(semesterId, confirm) {
        if (!confirm) {
    ...
    deleteAttendanceRecords(semesterId, confirm) {
        if (!confirm) {
            throw new ApiError(400, "VALIDATION_ERROR", "Confirmation required.");
        }
